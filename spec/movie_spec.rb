...
RSpec.describe 'Movie' do
  ...
  describe '#unwatched' do
    it 'pushes movie into unwatched array' do
      expect(movie.unwatched(movie.title)).to eq("#{movie} is unwatched.")
    end
  end

  describe '#watched' do
    it 'takes movie out of unwatched array, and put movie into watched array' do
      expect(movie.watched(movie.title)).to eq("#{movie} has been moved from unwatched to watched")
    end
    it 'does not add a movie to watched unless it is in the unwatched array' do
      expect(movie.watched(movie.title)).to eq("#{movie} not found")
    end
  end
end